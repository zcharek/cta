...
const QuizTestSelector: React.FC<QuizTestSelectorProps> = ({ modalOpen: modalOpenProp, setModalOpen: setModalOpenProp }) => {
  ...
  const handleIntroSubmit = (e: React.FormEvent) => {
    ...
    if (!introData.firstName.trim() || !introData.email.trim()) {
      setIntroError("Veuillez renseigner votre prénom et votre email pour continuer.");
      return;
    }
    setIntroStep(false);
  };
  ...
  return (
    ...
    <PatternBackground 
      variant="light" 
      opacity={0.1}
      className={`py-16 px-2 sm:px-0 webflow-section ${gradientBg}`}
    >
    <section id="quiz-test-selector">
      <div className="flex flex-col items-center justify-center my-8">
        <h2 className="text-2xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-brand-blue-900 to-brand-blue-500 mb-2 text-center">
        ...
        <button
          className="px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-brand-blue-900 via-brand-blue-800 to-brand-blue-500 text-white rounded-2xl font-bold text-base sm:text-lg shadow-lg hover:from-brand-blue-900 hover:to-brand-blue-600 transition-all mb-4 w-full max-w-xs"
          onClick={handleOpen}
        >
        ...
        {modalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
            <div className={`relative w-full max-w-xl mx-2 sm:mx-4 rounded-3xl shadow-2xl p-0 overflow-hidden ${gradientBg} animate-fade-in-up`} style={{maxHeight:'95vh'}}>
              {/* Bouton fermer */}
              <button
                className="absolute top-2 right-2 sm:top-4 sm:right-4 text-2xl sm:text-3xl text-gray-500 hover:text-brand-blue-600 font-bold z-10 bg-white/70 rounded-full w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center shadow"
                onClick={handleClose}
              ...
              <div className="h-2 w-full bg-gray-200">
                <div
                  className="h-2 bg-gradient-to-r from-brand-blue-900 via-brand-blue-500 to-brand-blue-400 rounded-full transition-all duration-500"
              ...
              <div className="p-4 sm:p-8 flex flex-col items-center overflow-y-auto" style={{maxHeight:'calc(95vh - 2rem)'}}>
                ...
                {introStep ? (
                  <form className="w-full max-w-md mx-auto flex flex-col gap-4" onSubmit={handleIntroSubmit}>
                    <h3 className="text-xl font-bold text-center mb-2 text-brand-blue-900">Commencer votre recommandation personnalisée</h3>
                    <input
                      type="text"
                      name="firstName"
                      placeholder="Prénom *"
                      className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-blue-600 text-base"
                    ...
                    <input
                      ...
                      onChange={handleIntroChange}
                    />
                    {introError && <p className="text-brand-blue-600 text-sm text-center">{introError}</p>}
                    <button
                      type="submit"
                      className="w-full mt-2 px-6 py-3 bg-gradient-to-r from-brand-blue-900 via-brand-blue-800 to-brand-blue-500 text-white rounded-xl font-bold text-base shadow-lg hover:from-brand-blue-900 hover:to-brand-blue-600 transition-all"
                    >
                      Poursuivre
                    </button>
                  </form>
                  ...
                  <div className="w-full">
                    <div className="mb-4 sm:mb-6">
                      ...
                      <div className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-center text-brand-blue-900">
                      ...
                      <div className="grid gap-3 sm:gap-4">
                        {questions[step].options.map((option) => (
                          <button
                            key={option}
                            className={`w-full px-4 sm:px-6 py-2 sm:py-3 rounded-xl border-2 transition-all duration-200 font-medium text-sm sm:text-base shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-blue-500 focus:ring-offset-2
                              ${answers[step] === option
                                ? "bg-gradient-to-r from-brand-blue-600 to-brand-blue-600 text-white border-brand-blue-600 scale-105 shadow-lg"
                                : "bg-white border-gray-300 hover:border-brand-blue-500 hover:bg-brand-blue-50 text-gray-800"}
                            `}
                    ...
                    {step > 0 && (
                      <button
                        className="text-brand-blue-900 underline text-xs sm:text-sm mt-2 hover:text-brand-blue-800 transition-all"
                  ...
                  <div className="text-center w-full flex flex-col items-center justify-center min-h-[350px]">
                    {sending ? (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="text-brand-blue-900 font-semibold text-lg">Envoi de votre recommandation personnalisée...</div>
                        <p className="text-gray-600 text-sm mt-2">Cela ne prendra que quelques secondes</p>
                      </div>
                    ) : sent ? (
                      <>
                        <div className="flex flex-col items-center justify-center mb-8">
                          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-brand-blue-500 to-brand-blue-600 flex items-center justify-center mb-4 shadow-xl animate-bounce">
                            <svg className="w-10 h-10 text-white" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                          </div>
                          <h3 className="text-2xl font-bold mb-4 text-brand-blue-900 tracking-tight">Votre recommandation personnalisée</h3>
                          <p className="text-brand-blue-900 mb-4">Voici la stratégie de test la plus adaptée à votre projet :</p>
                          <div className="bg-white/90 rounded-2xl px-10 py-8 shadow-2xl mb-8 flex flex-col items-center max-w-lg w-full">
                            <ul className="space-y-4 w-full">
                              {getRecommendation(answers).map((rec) => (
                                <li key={rec} className="flex items-center text-lg text-brand-blue-900 font-semibold">
                                  <span className="inline-block w-4 h-4 mr-4 bg-gradient-to-br from-brand-blue-600 to-brand-blue-600 rounded-full shadow"></span>
                                  {rec}
                                </li>
                              ))}
                            </ul>
                          ...
                          <button
                            className="mt-2 px-8 py-3 bg-gradient-to-r from-brand-blue-900 via-brand-blue-800 to-brand-blue-500 text-white rounded-2xl font-bold text-lg shadow-lg hover:from-brand-blue-900 hover:to-brand-blue-600 transition-all"
                            onClick={() => {
                              setModalOpen(false);
                              setTimeout(() => {
                      ...
                      <>
                        <div className="flex flex-col items-center justify-center mb-8">
                          ...
                          <h3 className="text-2xl font-bold mb-4 text-brand-blue-900 tracking-tight">Recommandation personnalisée</h3>
                        </div>
                        <div className="bg-white/90 rounded-2xl px-10 py-8 shadow-2xl mb-8 flex flex-col items-center max-w-lg w-full">
                          <ul className="space-y-4 w-full">
                            {getRecommendation(answers).map((rec) => (
    ...
    {/* Sticky button amélioré - rendu en dehors du PatternBackground pour éviter les problèmes de z-index */}
    <div
      className="fixed z-[99999] bottom-6 right-4 sm:bottom-8 sm:right-8 group isolate"
      style={{
    ...
    >
      <button
        ...
        className="flex items-center bg-gradient-to-r from-brand-blue-900 via-brand-blue-800 to-brand-blue-500 text-white rounded-full shadow-xl px-2 py-2 sm:px-3 sm:py-3 font-bold text-sm sm:text-base hover:scale-105 focus:scale-105 transition-all duration-200 outline-none animate-pulse-soft"
      >
        <span className="w-5 h-5 flex items-center justify-center text-lg font-bold bg-transparent">?</span>
