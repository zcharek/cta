        ...
        {modalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
            <div className={`relative w-full max-w-xl mx-2 sm:mx-4 rounded-3xl shadow-2xl p-0 overflow-hidden ${gradientBg} animate-fade-in-up`} style={{maxHeight:'95vh'}}>
              {/* Bouton fermer */}
              <button
                className="absolute top-2 right-2 sm:top-4 sm:right-4 text-2xl sm:text-3xl text-gray-500 hover:text-blue-500 font-bold z-10 bg-white/70 rounded-full w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center shadow"
                onClick={handleClose}
                ...
                {introStep ? (
                  <form className="w-full max-w-md mx-auto flex flex-col gap-4" onSubmit={handleIntroSubmit}>
                    ...
                    <input
                      ...
                      onChange={handleIntroChange}
                    />
                    {introError && <p className="text-blue-500 text-sm text-center">{introError}</p>}
                    <button
                      type="submit"
                        ...
                        {questions[step].options.map((option) => (
                          <button
                            ...
                            className={`w-full px-4 sm:px-6 py-2 sm:py-3 rounded-xl border-2 transition-all duration-200 font-medium text-sm sm:text-base shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2
                              ${answers[step] === option
                                ? "bg-gradient-to-r from-blue-500 to-blue-500 text-white border-blue-500 scale-105 shadow-lg"
                    ...
                    {sending ? (
                      ...
                      <>
                        <div className="flex flex-col items-center justify-center mb-8">
                          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-400 to-blue-500 flex items-center justify-center mb-4 shadow-xl animate-bounce">
                            <svg className="w-10 h-10 text-white" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                          </div>
                          <h3 className="text-2xl font-bold mb-4 text-blue-700 tracking-tight">Votre recommandation personnalisée</h3>
                            ...
                            <ul className="space-y-4 w-full">
                              {getRecommendation(answers).map((rec) => (
                                <li key={rec} className="flex items-center text-lg text-blue-900 font-semibold">
                                  <span className="inline-block w-4 h-4 mr-4 bg-gradient-to-br from-blue-500 to-blue-500 rounded-full shadow"></span>
                                  {rec}
                                </li>
                              ))}
                            </ul>
                      ...
                      <>
                        <div className="flex flex-col items-center justify-center mb-8">
                          ...
                          <h3 className="text-2xl font-bold mb-4 text-blue-700 tracking-tight">Recommandation personnalisée</h3>
                        </div>
                        <div className="bg-white/90 rounded-2xl px-10 py-8 shadow-2xl mb-8 flex flex-col items-center max-w-lg w-full">
                          <ul className="space-y-4 w-full">
                            {getRecommendation(answers).map((rec) => (
