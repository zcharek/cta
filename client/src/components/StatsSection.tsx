      ...
      <div className="container">
        <div className="flex flex-row flex-wrap justify-center items-center gap-2 sm:gap-4 md:gap-8 text-center">
          {stats.map((stat, index) => (
            ...
            >
                 <p className="text-2xl sm:text-3xl md:text-4xl font-bold mb-1 sm:mb-1.5 text-white">
                <AnimatedCounter target={stat.value} suffix={stat.suffix} />
              </p>
              <p className="text-sm sm:text-base md:text-lg font-medium leading-tight text-white">
                {stat.label}
              </p>
            </m.div>
          ))}
